...
router.get('/api/user/:id', isAuthenticated, async (req, res) => {
    const client = req.app.get('discordClient'); // Get client from main app
    try {
...
// Logic to recover missing tags from Discord Client cache
let fullDisplayList = [];
...
for (let i = 0; i < participants.length; i++) {
    ...
    if (tag) {
        // If we have the tag saved, use it
        fullDisplayList.push(tag);
    } else {
        // If tag is missing, try to find it in the bot's user cache
...
const participantList = fullDisplayList.length > 0 
    ? fullDisplayList.join(', ') 
    : "No entries yet";
        ...
        <html>
            <head>
                ...
                <style>
                    ...
                    td { padding: 10px; border-bottom: 1px solid #1a1a24; font-size: 0.85em; vertical-align: top; max-width: 200px; overflow: hidden; text-overflow: ellipsis; word-break: break-word; }
                </style>
            </head>
            <body>
                <div class="container">
                    ...
                    <div class="card" style="grid-column: 1 / -1;"> 
                        ...
                        <div style="overflow-x: auto;">
                            <table>
                                <thead>
                                    <tr><th>Manager</th><th>Rating</th><th>💚 Worked</th><th>💡 Improve</th><th>💬 Extra</th></tr>
                                </thead>
                                <tbody>
